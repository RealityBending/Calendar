...
# Define convenience function (needs to 'insol' and the 'lunar' package) to find dates of solstices, equinoxes, full moons and new moons for a given year.
get_dates_atronomical <- function(year=2020){
...
get_dates <- function(){
  ...
  if (celeb %in% c("Thanksday", "Moonfest")){
    season <- "Summer"
    period <- "Bright Days"
    phase <- "Fall"
  } else if (celeb %in% c("Harvestday", "Deadsnight")){
    season <- "Autumn"
    period <- "Dark Nights"
    phase <- "Fall"
    ...
    period <- "Dark Nights"
    phase <- "Fall"
  } else if (celeb %in% c("Preparationday", "Sunfest")){
    season <- "Winter"
    period <- "Dark Nights"
    phase <- "Rise"
  } else if (celeb %in% c("Loversnight", "Lifesday")){
    season <- "Spring"
    period <- "Bright Days"
    phase <- "Rise"
...
In the Reality Bending group, we follow the calendar of the phosphoranists. The year is divided in two main overlapping cycles, the **Rise/Fall** phase (December - June) and the **Dark nights/Bright days** period (September - March). 
