...
get_dates <- function(){
  ...
  if (celeb %in% c("Thanksday", "Fallfest", "Brightfall")){
    season <- "Summer"
    period <- "Bright Days"
    phase <- "Fall"
  } else if (celeb %in% c("Harvestday", "Deadsnight", "Darkfall", "Nightfest")){
    season <- "Autumn"
    period <- "Dark Nights"
    phase <- "Fall"
  } else if (celeb %in% c("Preparationday", "Risefest", "Darkrise")){
    season <- "Winter"
    period <- "Dark Nights"
    phase <- "Rise"
  } else if (celeb %in% c("Loversnight", "Lifesday", "Brightrise", "Lightfest")){
    season <- "Spring"
    period <- "Bright Days"
    phase <- "Rise"
  } else{
...
data <- data.frame(Day = days,
                   ...
                   Type = ifelse(days %in% celebrations[c("Risefest", "Fallfest", "Nightfest", "Lightfest",
                                                           "Darkrise", "Brightrise", "Darkfall", "Brightfall")], "Solar", 
                                 ifelse(days %in% celebrations[c("Blacksun")], "Other", "Lunar")))
...
data$Main <- ifelse(data$Day %in% celebrations[c("Risefest", "Fallfest", "Nightfest", "Lightfest",
...
**Fullmoon between Nightfest and Risefest** `r format(celebrations["Preparationday"], "*(%Y %B %d - %A)*")`. Team building and planning of the work roadmap for the year. Winter retreat.

### Risefest

**Spring equinox** `r format(celebrations["Risefest"], "*(%Y %B %d - %A)*")`. Coming of **Bright days** and **Spring**. Drowning of the *Winter Woman* ([Marzanna](https://wpna.fm/polish-traditions-the-drowning-of-marzanna/)). Eating eggs and water pranks. 

### Loversnight

**Fullmoon between Risefest and Lightfest** `r format(celebrations["Loversnight"], "*(%Y %B %d - %A night)*")`. Celebration of partners and loved ones.
...
**Fullmoon between Lightfest and Fallfest** `r format(celebrations["Thanksday"], "*(%Y %B %d - %A)*")`. Work superiors treat to dinner. Rewards and vacations.

### Fallfest

**Autumn equinox** `r format(celebrations["Fallfest"], "*(%Y %B %d - %A)*")`. 
Coming of the **Dark Nights** and **Autumn**. Beginning of the school year.
Fires are lit (no electricity). [Wreaths](https://en.wikipedia.org/wiki/Do%C5%BCynki) symbolizing the successful readiness for the year to come. We go on a walk and collect materials to create along the way the *Winter Woman* doll, that will stay in the lab until Risefest.

### Harvestday

**Day before the fullmoon between Fallfest and Risefest** `r format(celebrations["Harvestday"], "*(%Y %B %d - %A)*")`. 
Physical/sports games and competitions (hackathons) between colleagues with silly costumes.

### Deadsnight

**Fullmoon between Fallfest and Nightfest** `r format(celebrations["Deadsnight"], "*(%Y %B %d - %A)*")`. Visit cemeteries. Start of **Winter**.

### Blacksun

- *The week before Nightfest* `r format(celebrations["Blacksun"], "*(%Y %B %d - %A)*")`. When darkness is winning: Chaos and mischief. Elect a "Lord of Misrule". Inversed hierarchy (see [Saturnalia](https://en.wikipedia.org/wiki/Saturnalia)).
